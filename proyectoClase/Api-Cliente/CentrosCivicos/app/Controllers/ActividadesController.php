...
class ActividadesController {
        ...
        switch ($this->requestMethod) {
            case 'GET':
                if($this->centrosCivicosId){
                    $response = $this->getActividades($this->centrosCivicosId);
                    break;
                }else{
                    $response = $this->getActividadesFiltro();
                    break;
                }
            case 'POST':
                break;
    ...
    private function getActividadesFiltro(){
        $input = (array) json_decode(file_get_contents('php://input') , true);
        $result = $this->actividades->getFiltro($input);
        if(!$result){
            return $this->notFoundResponse();
        }
        $response['status_code_header'] = 'HTTP/1.1 200 OK';
