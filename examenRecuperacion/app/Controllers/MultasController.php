...
class MultasController extends BaseController
{
    ...
    public function NuevaMultaAction(){
        ...
        $data['id_agente'] = $usuarios->getAgentes($_SESSION['user_id']);
        ...
        if(isset($_POST['enviar'])){
            ...
            if($data['id_agente'] == ''){
                $lProcesaFormulario = false;
                $data['msjErrorIdAgente'] = 'El ID del agente no es correcto';
            }
            ...
            if($lProcesaFormulario){
                ...
                var_dump($data['id_agente']);
                $multas->setIdAgente($data['id_agente']['id']);
